...
export const parsePostAbstract = (postPath: string) => {
  ...
  // categoryPath의 _를 스페이스로 바꾸고 소문자로 변환
  const convertedCategoryPath = categoryPath.replace(/_/g, ' ').toLowerCase();
  const url = `/post/${convertedCategoryPath.replace(/ /g, '_')}/${slug}`;
